...
use latch_protocol::{
    ActRequest, ActionOutcome, AgentRequest, BrowserRequest, DirectoryEntryResponse,
...
impl Engine {
    ...
    fn agent_session(
        ...
        match request {
            ...
            SessionRequest::Inspect { session_id } => to_value(
                self.sessions
                    .inspect(session_id)
        ...
        match request {
            InspectRequest::Windows { session_id, limit } => {
                ...
                self.authorize(
                    config,
                    Some(session_id),
                    Capability::UiInspection,
                    "Inspect desktop windows",
                )?;
                let windows = self
                    .windows_provider()?
                    .desktop_windows(session_id, limit.unwrap_or(32))
            ...
            InspectRequest::UiRef {
                ...
                self.authorize(
                    ...
                    Some(session_id),
                    Capability::UiInspection,
                    "Inspect Windows UI element",
                )?;
                let element = self
                    .windows_provider()?
                    .inspect(session_id, element_ref)
                    .map_err(map_windows_error)?;
            ...
            InspectRequest::Applications { session_id, limit } => {
                ...
                let apps = self
                    .windows_provider()?
                    .applications(session_id, limit.unwrap_or(64))
                    .map_err(map_windows_error)?;
            ...
            InspectRequest::Audio { session_id } => {
                ...
                self.authorize(
                    config,
                    Some(session_id),
                    Capability::NativeSystemControl,
                    "Read Windows volume",
                )?;
                let audio = self
                    .windows_provider()?
    ...
    fn agent_files(
        ...
        match request {
            ...
            FilesRequest::OpenWorkspace {
                ...
                let response =
                    self.open_approved_workspace(config, root_id, relative_path.as_deref())?;
                to_value(response)
            }
            ...
            FilesRequest::Write {
                ...
                context
                    .fs
                    .write_text_with_mode(&path, &contents, overwrite)
                    .map_err(map_fs_error)?;
            ...
            FilesRequest::Patch {
                ...
                context
                    .fs
                    .apply_replacements(&path, &mapped)
                    .map_err(map_fs_error)?;
                let verified = if verification == VerificationMode::None {
                    None
                } else {
    ...
    fn agent_exec(
        ...
        match request {
            ExecDomainRequest::Run {
                ...
                let context = self.workspace(config, workspace_id)?;
                let result =
                    run_blocking(&context.workspace, &CommandSpec::new(program.clone(), args))
            ...
            ExecDomainRequest::Start {
                ...
                let started = self
                    .processes
                    .start_detailed(&context.workspace, &CommandSpec::new(program.clone(), args))
                    .map_err(map_exec_error)?;
                self.sessions
                    .bind_process(session_id, started.process_id)
                    .map_err(map_session_error)?;
                self.events.publish(
                    session_id,
                    "process",
                    ...
                    Some(json!({"job_id":started.process_id,"pid":started.pid})),
                );
                to_value(
                    json!({"route_used":"process","job_id":started.process_id,"pid":started.pid,"started_at_ms":started.started_at_ms}),
                )
            }
            ExecDomainRequest::Poll { session_id, job_id } => {
                ...
                if !matches!(poll.status.state, ProcessState::Running) {
                    self.events.publish(
                        ...
                        "Process exited",
                        Some(
                            json!({"job_id":job_id,"state":map_process_state(&poll.status.state)}),
                        ),
                    );
                }
            ...
            ExecDomainRequest::Stdin {
                ...
                self.processes
                    .write_stdin(job_id, &text, close_stdin)
                    .map_err(map_exec_error)?;
            ...
            ExecDomainRequest::TerminalCreate {
                ...
                self.sessions
                    .bind_terminal(session_id, terminal.terminal_id)
                    .map_err(map_session_error)?;
                self.events.publish(
                    session_id,
                    "terminal",
                    "terminal.created",
                    "Persistent terminal created",
                    Some(json!({"terminal_id":terminal.terminal_id,"pid":terminal.pid})),
                );
                to_value(json!({"route_used":"conpty","terminal":terminal}))
            }
            ExecDomainRequest::TerminalWrite {
                session_id,
                terminal_id,
                text,
            } => {
                ...
                self.authorize(
                    ...
                    Some(session_id),
                    Capability::Terminal,
                    "Write persistent terminal",
                )?;
                self.terminals
                    .write(terminal_id, &text)
                    .map_err(map_terminal_error)?;
                to_value(json!({"route_used":"conpty","accepted":true}))
            }
            ExecDomainRequest::TerminalRead {
                session_id,
                terminal_id,
                after_sequence,
                max_bytes,
                ...
                self.authorize(
                    ...
                    Some(session_id),
                    Capability::Terminal,
                    "Read persistent terminal",
                )?;
                let snapshot = self
                    .terminals
                    .snapshot(terminal_id, after_sequence, max_bytes)
                    .map_err(map_terminal_error)?;
                if !snapshot.output.is_empty() {
                    self.events.publish(
                        ...
                        Some(json!({"terminal_id":terminal_id,"sequence":snapshot.sequence})),
                    );
                }
            ...
            ExecDomainRequest::TerminalResize {
                session_id,
                terminal_id,
                rows,
                cols,
            ...
            ExecDomainRequest::TerminalInterrupt {
                session_id,
                terminal_id,
            } => {
                self.require_owned_terminal(session_id, terminal_id)?;
            ...
            ExecDomainRequest::TerminalKill {
                session_id,
                terminal_id,
            } => {
                self.require_owned_terminal(session_id, terminal_id)?;
            ...
            ExecDomainRequest::TerminalList { session_id } => {
                ...
                let terminals = self
                    ...
                    .filter(|terminal| {
                        self.sessions
                            .owns_terminal(session_id, terminal.terminal_id)
                    })
                    .collect::<Vec<_>>();
    ...
    fn agent_act(&self, config: &LocalConfig, request: ActRequest) -> Result<Value, ProtocolError> {
        match request {
            ActRequest::AppLaunch {
                ...
                let launched = self
                    .windows_provider()?
                    .launch_application(&program, &args, cwd.as_deref())
                    .map_err(map_windows_error)?;
                let verified = if verification == VerificationMode::None {
                    None
                } else {
                ...
                self.events.publish(
                    ...
                    "windows",
                    "window.opened",
                    &format!("Launched {program}"),
                    Some(json!({"pid":launched.pid})),
                );
                self.verified_value(
                    "windows_app",
                    verification,
                    verified,
                    vec![launched.pid.to_string()],
                    json!({"launch":launched}),
                )
            }
            ActRequest::AppActivate {
                session_id,
                pid,
                verification,
            } => {
            ...
            ActRequest::AppQuit {
                ...
                self.windows_provider()?
                    .quit_application(pid)
                    .map_err(map_windows_error)?;
                let verified = if verification == VerificationMode::None {
                    None
                } else {
                    Some(self.wait_for_app(session_id, pid, false))
                };
                self.events.publish(
                    session_id,
                    "windows",
            ...
            ActRequest::AudioSet {
                ...
                self.authorize(
                    ...
                    Some(session_id),
                    Capability::NativeSystemControl,
                    &format!("Set Windows volume to {volume_percent}%"),
                )?;
                let audio = self
                    .windows_provider()?
                    .audio_set(volume_percent)
                    .map_err(map_windows_error)?;
                let verified = if verification == VerificationMode::None {
                    None
                } else {
                    Some(audio.volume_percent.abs_diff(volume_percent) <= 1)
                };
                self.verified_value(
                    "windows_audio",
                    verification,
            ...
            ActRequest::Ui {
                ...
                self.authorize(
                    ...
                    "Control Windows UI element",
                )?;
                let result = self
                    .windows_provider()?
                    .act(session_id, element_ref, map_ui_action(action))
                    .map_err(map_windows_error)?;
                let verified = if verification == VerificationMode::None {
                    None
                } else {
                ...
                self.verified_value(
                    ...
                    verification,
                    verified,
                    vec![element_ref.to_string()],
                    json!({"element":result.element}),
                )
            }
            ActRequest::Screenshot {
                session_id,
                display_id,
                format,
            } => {
                ...
                let screenshot = lock(&self.computer)
                    .screenshot(display_id.as_deref(), format)
            ...
            ActRequest::RawInput { session_id, input } => {
                ...
                self.authorize(
                    ...
                    Some(session_id),
                    Capability::RawInput,
                    "Send raw keyboard or mouse input",
                )?;
                let computer = lock(&self.computer);
                match input {
                    RawInputRequest::MouseMove { x, y } => computer.mouse_move(x, y),
                    RawInputRequest::MouseClick { button } => {
                        computer.mouse_click(parse_mouse_button(&button)?)
                    }
                    RawInputRequest::MouseDrag {
                        from_x,
                        from_y,
                        to_x,
                        to_y,
                    ...
                    RawInputRequest::Scroll { amount, axis } => {
                        computer.scroll(amount, parse_scroll_axis(&axis)?)
                    }
        ...
        match request {
            BrowserRequest::Status { session_id } => {
                ...
                self.authorize(
                    config,
                    Some(session_id),
                    Capability::BrowserIsolated,
                    "Inspect browser runtime",
                )?;
                to_value(
                    json!({"route_used":"playwright","status":self.browser.status().map_err(map_browser_error)?}),
                )
            }
            BrowserRequest::CreateContext {
                session_id,
                authenticated,
                persistent,
            } => {
                self.require_session(session_id)?;
                let capability = if authenticated {
                    Capability::BrowserAuthenticated
                } else {
                    Capability::BrowserIsolated
                };
                self.authorize(
                ...
                let context = self
                    .browser
                    .create_context(profile)
                    .map_err(map_browser_error)?;
                self.sessions
                    .bind_context(session_id, context.context_id)
                    .map_err(map_session_error)?;
            ...
            BrowserRequest::ListContexts { session_id } => {
                ...
                let contexts = self
                    ...
                    .map_err(map_browser_error)?
                    .into_iter()
                    .filter(|context| self.sessions.owns_context(session_id, context.context_id))
                    .collect::<Vec<_>>();
                to_value(json!({"route_used":"playwright","contexts":contexts}))
            }
            BrowserRequest::CloseContext {
                session_id,
                context_id,
            } => {
            ...
            BrowserRequest::NewTab {
                ...
                let tab = self
                    .browser
                    .new_tab(context_id, url.as_deref())
                    .map_err(map_browser_error)?;
                self.sessions
                    .bind_tab(session_id, tab.tab_id)
                    .map_err(map_session_error)?;
                self.events.publish(
                    session_id,
                    "browser",
                    "browser.navigation",
                    "Browser tab opened",
                    Some(json!({"tab_id":tab.tab_id,"url":tab.url})),
                );
            ...
            BrowserRequest::ListTabs {
                ...
                let tabs = self
                    .browser
                    .list_tabs(context_id)
                    .map_err(map_browser_error)?
                    .into_iter()
                    .filter(|tab| self.sessions.owns_tab(session_id, tab.tab_id))
                    .collect::<Vec<_>>();
            ...
            BrowserRequest::Navigate {
                ...
                self.events.publish(
                    ...
                    "browser.navigation",
                    "Browser navigated",
                    Some(json!({"tab_id":tab_id,"url":tab.url})),
                );
                to_value(json!({"route_used":"playwright","tab":tab}))
            }
            BrowserRequest::Snapshot { session_id, tab_id } => {
                ...
                self.authorize(
                    ...
                    Some(session_id),
                    Capability::BrowserIsolated,
                    "Inspect browser semantics",
                )?;
                to_value(
                    json!({"route_used":"playwright","snapshot":self.browser.snapshot(tab_id).map_err(map_browser_error)?}),
                )
            }
            BrowserRequest::Find {
                session_id,
                ...
                let target: BrowserTarget = serde_json::from_value(target).map_err(|error| {
                    protocol_error(ErrorCode::InvalidRequest, error.to_string())
                })?;
                to_value(
                    json!({"route_used":"playwright","elements":self.browser.find(tab_id, target, max_results.unwrap_or(10)).map_err(map_browser_error)?}),
                )
            }
            BrowserRequest::Act {
                session_id,
                ...
                let action: BrowserAction = serde_json::from_value(action).map_err(|error| {
                    protocol_error(ErrorCode::InvalidRequest, error.to_string())
                })?;
                let browser_verification: Option<BrowserVerification> = browser_verification
                ...
                let result = self
                    .browser
                    .act(tab_id, target, action, browser_verification)
                    .map_err(map_browser_error)?;
                let verified = if verification == VerificationMode::None {
                    None
                } else {
            ...
            BrowserRequest::Console {
                ...
                let entries = self
                    .browser
                    .console(tab_id, after_sequence, max_entries.unwrap_or(50))
                    .map_err(map_browser_error)?;
                if !entries.is_empty() {
                    self.events.publish(
                        session_id,
                        ...
                        Some(json!({"tab_id":tab_id,"count":entries.len()})),
                    );
                }
            ...
            BrowserRequest::Network {
                ...
                self.authorize(
                    ...
                    Some(session_id),
                    Capability::BrowserIsolated,
                    "Read browser network activity",
                )?;
                let entries = self
                    .browser
                    .network(tab_id, after_sequence, max_entries.unwrap_or(50))
                    .map_err(map_browser_error)?;
                if entries.iter().any(|entry| entry.kind == "request_failed") {
                    self.events.publish(
                        session_id,
                        "browser",
            ...
            BrowserRequest::Downloads {
                ...
                self.authorize(
                    ...
                    Some(session_id),
                    Capability::BrowserIsolated,
                    "Read browser downloads",
                )?;
                let entries = self
                    .browser
                    .downloads(tab_id, after_sequence, max_entries.unwrap_or(50))
                    .map_err(map_browser_error)?;
                if !entries.is_empty() {
                    self.events.publish(
                        session_id,
            ...
            BrowserRequest::Screenshot { session_id, tab_id } => {
                ...
                to_value(
                    json!({"route_used":"playwright","screenshot":self.browser.screenshot(tab_id).map_err(map_browser_error)?}),
                )
            }
            BrowserRequest::PageState { session_id, tab_id } => {
                ...
                to_value(
                    json!({"route_used":"playwright","page":self.browser.page_state(tab_id).map_err(map_browser_error)?}),
                )
            }
        }
    }
        ...
        match request {
            ToolsRequest::Providers { session_id } => {
                ...
                self.authorize(
                    config,
                    Some(session_id),
                    Capability::McpDiscovery,
                    "Inspect local MCP providers",
                )?;
                to_value(
                    json!({"route_used":"mcp_federation","providers":self.mcp.providers(&config.mcp_servers).map_err(map_mcp_error)?}),
                )
            }
            ToolsRequest::Search {
                session_id,
            ...
            ToolsRequest::Describe {
                ...
                self.authorize(
                    ...
                    Some(session_id),
                    Capability::McpDiscovery,
                    "Describe local MCP tool",
                )?;
                let tool = self
                    .mcp
                    .describe(&config.mcp_servers, tool_ref)
                    .map_err(map_mcp_error)?;
            ...
            ToolsRequest::Call {
                ...
                self.authorize(
                    config,
                    Some(session_id),
                    Capability::McpExecution,
                    "Execute local MCP tool",
                )?;
                let result = self
                    .mcp
                    .call(&config.mcp_servers, tool_ref, arguments)
                    .map_err(map_mcp_error)?;
                self.events.publish(
                    session_id,
                    "mcp",
                    "mcp.called",
                    "Local MCP tool completed",
                    Some(json!({"tool_ref":tool_ref})),
                );
    ...
    fn authorize(
        ...
        match config.capability_policy.get(capability) {
            ...
            PermissionMode::Ask => {
                if self
                    .local
                    .consume_approval_grant(session_id, capability, summary)
                    .map_err(map_local_error)?
                {
                    return Ok(());
                }
                let approval = self
                    .local
                    .queue_approval(session_id, capability, summary)
                    .map_err(map_local_error)?;
                ...
                Err(protocol_error(
                    ...
                    format!(
                        ...
                        approval.approval_id, approval.summary
                    ),
                ))
            }
        }
    ...
    fn require_session_workspace(
        ...
        if self.sessions.owns_workspace(session_id, workspace_id) {
            ...
            Err(protocol_error(
                ErrorCode::PermissionDenied,
                "workspace is not bound to this Latch session",
            ))
        }
    }
    ...
    fn require_owned_terminal(
        ...
        self.require_session(session_id)?;
        if self.sessions.owns_terminal(session_id, terminal_id) {
            Ok(())
        } else {
            Err(protocol_error(
                ErrorCode::TerminalNotFound,
                "terminal is not owned by this session",
            ))
        }
    }

    fn require_owned_process(
        &self,
        session_id: SessionId,
        process_id: ProcessId,
    ) -> Result<(), ProtocolError> {
        self.require_session(session_id)?;
        if self.sessions.owns_process(session_id, process_id) {
            Ok(())
        } else {
            Err(protocol_error(
                ErrorCode::ProcessNotFound,
                "process is not owned by this session",
            ))
        }
    }
    ...
    fn require_owned_context(
        &self,
        session_id: SessionId,
        context_id: latch_core::BrowserContextId,
    ) -> Result<(), ProtocolError> {
        self.require_session(session_id)?;
        if self.sessions.owns_context(session_id, context_id) {
            Ok(())
        } else {
            Err(protocol_error(
                ErrorCode::BrowserUnavailable,
                "browser context is not owned by this session",
            ))
        }
    }
    ...
    fn require_owned_tab(
        &self,
        session_id: SessionId,
        tab_id: latch_core::TabId,
    ) -> Result<(), ProtocolError> {
        self.require_session(session_id)?;
        if self.sessions.owns_tab(session_id, tab_id) {
            Ok(())
        } else {
            Err(protocol_error(
                ErrorCode::BrowserUnavailable,
                "browser tab is not owned by this session",
            ))
        }
    }
    ...
    fn windows_provider(&self) -> Result<&WindowsManager, ProtocolError> {
        self.windows.as_ref().ok_or_else(|| {
            protocol_error(
                ...
                "Windows semantic provider is unavailable",
            )
        })
    }
    ...
    fn wait_for_app(&self, session_id: SessionId, pid: u32, expected_present: bool) -> bool {
        ...
        for _ in 0..10 {
            let present = windows
                .applications(session_id, 64)
                .is_ok_and(|apps| apps.iter().any(|app| app.pid == pid));
    ...
    fn open_approved_workspace(
        ...
        write_lock(&self.workspaces).insert(
            workspace_id,
    ...
    fn open_raw_workspace_internal(
        &self,
        config: &LocalConfig,
        path: String,
    ) -> Result<WorkspaceResponse, ProtocolError> {
    ...
    fn workspace(
        ...
        config: &LocalConfig,
        workspace_id: WorkspaceId,
    ) -> Result<Arc<WorkspaceContext>, ProtocolError> {
        let context = read_lock(&self.workspaces)
            .get(&workspace_id)
            .cloned()
            .ok_or_else(|| {
                protocol_error(
    ...
    fn open_workspace_legacy(
        &self,
        config: &LocalConfig,
        request: WorkspaceOpenRequest,
    ) -> Result<ResponsePayload, ProtocolError> {
        require_legacy(config.permissions.files, "file access is disabled locally")?;
        Ok(ResponsePayload::Workspace(self.open_approved_workspace(
            config,
            request.root_id,
    ...
    fn open_raw_workspace_legacy(
        ...
        Ok(ResponsePayload::Workspace(
            self.open_raw_workspace_internal(config, request.path)?,
        ))
    }
    ...
    fn list_dir_legacy(
        ...
        let entries = self
            .workspace(config, request.workspace_id)?
            .fs
    ...
    fn write_file_legacy(
        ...
        validate_remote_path(&request.path)?;
        self.workspace(config, request.workspace_id)?
            .fs
    ...
    fn run_command_legacy(
        ...
        let result = run_blocking(
            &self.workspace(config, request.workspace_id)?.workspace,
            &CommandSpec::new(request.program, request.args),
        )
        .map_err(map_exec_error)?;
    ...
    fn displays_legacy(
        ...
        let displays = lock(&self.computer)
            .displays()
            .map_err(map_computer_error)?
            .into_iter()
    ...
    fn windows_legacy(
        ...
        let windows = lock(&self.computer)
            .windows()
            .map_err(map_computer_error)?
            .into_iter()
    ...
    fn focus_window_legacy(
        &self,
        config: &LocalConfig,
        request: WindowRequest,
    ) -> Result<ResponsePayload, ProtocolError> {
        require_legacy(
            config.permissions.computer_control,
            "computer control is disabled locally",
        )?;
        lock(&self.computer)
    ...
    fn mcp_servers_legacy(
        &self,
        config: &LocalConfig,
        _request: EmptyRequest,
    ) -> Result<ResponsePayload, ProtocolError> {
        require_legacy(
            config.permissions.mcp_discovery,
            "local MCP discovery is disabled locally",
        )?;
        let providers = self
            .mcp
            .providers(&config.mcp_servers)
            .map_err(map_mcp_error)?;
        Ok(ResponsePayload::McpServers(McpServersResponse {
            servers: providers
                .into_iter()
                .map(|provider| McpServerResponse {
                    server_id: provider.server_id,
                    display_name: provider.display_name,
                    status: if provider.connected {
                        McpServerStatusResponse::Connected
                    } else {
                        McpServerStatusResponse::Degraded
                    },
                })
                .collect(),
        }))
    ...
    fn mcp_call_legacy(
        ...
        require_legacy(
            config.permissions.mcp_execution,
...
fn require_legacy(enabled: bool, message: &str) -> Result<(), ProtocolError> {
    if enabled {
        ...
        Err(protocol_error(ErrorCode::PermissionDenied, message))
    }
}
...
fn parse_scroll_axis(value: &str) -> Result<ScrollAxis, ProtocolError> {
    match value.to_ascii_lowercase().as_str() {
        ...
        _ => Err(protocol_error(
            ErrorCode::InvalidRequest,
            "scroll axis must be vertical or horizontal",
        )),
    }
}
...
fn parse_screenshot_format(value: Option<&str>) -> Result<ScreenshotFormat, ProtocolError> {
    match value.unwrap_or("jpeg").to_ascii_lowercase().as_str() {
        ...
        _ => Err(protocol_error(
            ErrorCode::InvalidRequest,
            "screenshot format must be jpeg, webp, or png",
        )),
    }
}
...
fn map_exec_error(error: ExecError) -> ProtocolError {
    let code = match error {
        ...
        ExecError::PermissionDenied { .. } => ErrorCode::PermissionDenied,
...
fn map_terminal_error(error: TerminalError) -> ProtocolError {
    let code = match error {
        TerminalError::NotFound(_) | TerminalError::NotRunning(_) => ErrorCode::TerminalNotFound,
        TerminalError::PayloadTooLarge => ErrorCode::PayloadTooLarge,
        TerminalError::ProfileNotFound(_) | TerminalError::Create(_) => ErrorCode::InvalidRequest,
        TerminalError::Io(_) => ErrorCode::Io,
    };
    protocol_error(code, error.to_string())
}
...
fn map_windows_error(error: WindowsError) -> ProtocolError {
    let code = match error {
        WindowsError::Unsupported
        | WindowsError::UiUnavailable(_)
        | WindowsError::WorkerStopped => ErrorCode::ComputerUnavailable,
        WindowsError::StaleRef | WindowsError::RefSessionMismatch => ErrorCode::UiRefStale,
        WindowsError::TargetElevated => ErrorCode::TargetElevated,
        WindowsError::SecureDesktop => ErrorCode::SecureDesktop,
        WindowsError::InvalidInput(_) => ErrorCode::InvalidRequest,
        WindowsError::Application(_) | WindowsError::Clipboard(_) | WindowsError::Audio(_) => {
            ErrorCode::ComputerUnavailable
        }
    };
    protocol_error(code, error.to_string())
...
fn map_browser_error(error: BrowserError) -> ProtocolError {
    let code = match error {
        BrowserError::InvalidInput(_) => ErrorCode::InvalidRequest,
        BrowserError::Unavailable(_)
        | BrowserError::Protocol(_)
        | BrowserError::Operation(_)
        | BrowserError::Io(_) => ErrorCode::BrowserUnavailable,
    };
    protocol_error(code, error.to_string())
}
...
fn map_session_error(error: String) -> ProtocolError {
    let code = if error.contains("closed") {
...
mod tests {
    ...
    fn engine_with_root() -> (tempfile::TempDir, Engine, latch_core::RootId) {
        ...
        store
            .set_capability_policy(PermissionPolicy::preset(PermissionPreset::Developer))
            .unwrap();
    ...
    fn sessions_bind_workspace_and_verified_file_write() {
        ...
        let session: SessionSnapshot = serde_json::from_value(
            match engine
                .handle(Request::Agent(AgentRequest::Session(
                    SessionRequest::Create,
                )))
                .unwrap()
            {
                ResponsePayload::Agent(value) => value,
                _ => panic!("wrong payload"),
            },
        )
        .unwrap();
        let workspace: WorkspaceResponse = serde_json::from_value(
            match engine
                .handle(Request::Agent(AgentRequest::Files(
                    FilesRequest::OpenWorkspace {
                        root_id,
                        relative_path: None,
                    },
        ...
        engine
            ...
            .unwrap();
        let response = engine
            .handle(Request::Agent(AgentRequest::Files(FilesRequest::Write {
                workspace_id: workspace.workspace_id,
                path: "test.txt".to_owned(),
                contents: "Latch".to_owned(),
                overwrite: true,
                verification: VerificationMode::Required,
            })))
            .unwrap();
        let ResponsePayload::Agent(value) = response else {
            panic!("wrong payload")
        };
    ...
    fn denied_permission_does_not_fallback() {
        ...
        let error = engine
            .agent_act(
                &engine.local.load().unwrap(),
                ActRequest::Ui {
                    session_id: session.session_id,
                    element_ref: UiRef::new(),
                    action: UiActionRequest::Invoke,
