...
$latch = Get-Command latch -ErrorAction SilentlyContinue
if (-not $latch) {
    $candidate = Join-Path $PSScriptRoot '..\target\release\latch-link.exe'
    if (Test-Path -LiteralPath $candidate) { $latch = Get-Item $candidate }
}
Check 'Latch installation' { if (-not $latch) { throw 'latch is not installed or on PATH' }; & $latch.Source --version }
...
Check 'Background status' { & $latch.Source status | Out-String }
if ($failed) { exit 1 }
